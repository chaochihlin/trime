...
class T9KeyboardView @JvmOverloads constructor(
    ...
    private fun setupDynamicLayout() {
        ...
        for (i in 0 until 9) {
            ...
            add(numberKeys[i], lParams(dynamicKeySize, dynamicKeySize) {  // 使用動態尺寸
                when (col) {
                    0 -> {
        ...
        add(zeroKey, lParams(0, dp(FUNCTION_KEY_HEIGHT_DP)) {  // wrap-content寬度
            topToBottomOf(numberKeys[6], dp(KEY_SPACING_DP * 2))
            bottomOfParent(dp(4))  // 置底
            constrainedWidth = true
        })

        add(languageKey, lParams(0, dp(FUNCTION_KEY_HEIGHT_DP)) {  // wrap-content寬度
